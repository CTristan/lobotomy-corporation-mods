...
- **90–94, keystone.** The clearest, most directly liftable references: Slay the Spire (90) and Into the Breach (90).
- **85–89, strong.** Directly liftable: Hades (88), FTL (88), Left 4 Dead (88), Darkest Dungeon (87), Blue Prince (87), dotAGE (86), RimWorld (86), Diablo affixes (85), and Risk of Rain (85).
- **80–84, solid.** Narrower or partly redundant: Spelunky (82), Balatro (82), Monster Train (80), and Caves of Qud (80).
...
- **Forced draft, no skip.** You can't decline the pick, and each piece welds a benefit to a risk, so the tension is structural rather than opt-in.
...
- **Research:**
  - **Slay the Spire (90).** It's still the keystone for this lever because card value is contextual, so a card that's dead weight in one deck is a build-around in another, which means the variance lives in how the pieces interact rather than in the pieces themselves. Study how draft offers are conditioned on what you already hold and how relics swing the value of whole archetypes, because that conditioning is what the blind-pick POC has to reproduce. Two things won't transfer: the 350-plus card pool that keeps it fresh, which LobCorp's far smaller roster can't lean on, and the skippable, net-positive draft it's balanced around, which is the fidelity gap that eases it to 90, since you can't decline LobCorp's draft and every abnormality is double-edged.
  - **Blue Prince (87).** It's the reference that actually models LobCorp's draft, because its room draft is forced and double-edged, you can't leave a draft without placing one of three rooms and several rooms carry outright penalties, so the tension is structural the way a forced abnormality draft's is. Study how it gates and sequences that forced draft across a day, because both the day-pacing and the no-skip structure lift directly onto the POC. Its failure mode is the caution, because Blue Prince keeps rolling RNG that blocks you from applying knowledge you already earned, which breaks the no-unrecoverable-corner principle, so study it as much for what to avoid as for what to copy.
  - **Against the Storm (86).** It's the management-genre proof for this lever, and it holds its 86 where the purely skippable references eased, because each settlement drafts its production buildings and cornerstone perks, several of them double-edged, from randomized offers, so the loadout recombines run to run and a cornerstone you decline for Amber still cost you something. Study how the blueprint and cornerstone offers condition on the three clans and the resources you already hold, because that in-genre conditioning is the closest working model for the abnormality draft the POC has to build. The real-time settlement sim won't transfer, because Against the Storm recombines buildings across a live economy and LobCorp resolves work day by day, so the draft-and-interact lesson lifts while the moment-to-moment pacing doesn't.
  - **Balatro (82).** It earns its tier here by getting emergent synergy from a tiny fixed set, the eight poker hands, recombined through a 150-plus joker pool, which proves a small memorizable base stays novel when the modifiers run deep. Study the joker-to-hand interaction, because the lesson is that the modifiers carry the variance while the base set stays known, which maps straight onto abnormalities-plus-affixes. It eases to 82 because its draft is the most skip-designed of the references, since a joker literally rewards skipping packs, and the pure-score loop won't transfer either, because Balatro only asks you to beat a rising chip threshold while LobCorp runs need objectives with texture.
- **LobCorp:** the conditioned-draft, blind-pick, lockdown work, which is the POC. Blind-pick hides *which* abnormality you drew, but it has to keep the stakes legible (the risk tier, the rough category), or it breaks the Into the Breach rule. Two things separate LobCorp's draft from most of these references: you can't decline it, and every abnormality welds a benefit to a risk. So the recombination lesson lifts from all of them, but the skip-as-a-lever and pieces-are-upside assumptions don't, which is why Blue Prince's forced, double-edged room draft now leads them here as the closest structural model. That's a property to design around, not a gap to close, because a forced double-edged draft imposes more variance than a skippable one, which is the direction replayability wants.
...
- **Changes under the applicability criterion:** Dwarf Fortress moved from a competitive entry to a foundational reference (a great idea, but unbuildable, so its value lives in RimWorld and Qud). Left 4 Dead's AI Director was added as the more applicable director model. Caves of Qud rose to 80 once its lesson resolved into the cheap, liftable procedural-history technique. The score bands were introduced and made revisable, and Into the Breach moved from the strong band to keystone (90) without changing its number. Per-lever scoring was added, so each game now carries a per-approach score alongside its game-level score, and that per-lever score orders the reference lists. It moved Diablo-style affixes above dotAGE and Against the Storm in Approach 2, because affixed content is Diablo's whole identity but a secondary application for the two management games. No game-level score changed. The per-section count cap was then removed, so a reference earns its slot by clearing the 80 per-lever floor without being redundant, and no fixed count limits a section. Approach 1 was then re-rated with a forced/double-edged draft criterion weighted co-equally with recombination, which re-ranked it: Blue Prince rose from 83 to 87 as the only reference that models a forced, double-edged draft, Slay the Spire eased from 92 to 90 and Balatro from 84 to 82 for their skippable, net-positive drafts, and Monster Train fell to 79 and out of Approach 1 while holding its 80 in Approach 3. Slay the Spire's game-level score followed its Approach 1 peak down to 90, tying Into the Breach at the keystone floor and collapsing its Approach 3 bullet to a single 90.
...
- **rev 8, 2026-07-05.** Re-rated and re-ranked Approach 1 after adding a forced/double-edged draft
  criterion, from an observation rather than a game. Verified the references split on draft fidelity:
  Slay the Spire, Balatro, and Monster Train are skippable and net-positive, Against the Storm keeps a
  skip-for-Amber valve over a partly double-edged cornerstone pool, and Blue Prince's room draft is
  fully forced and double-edged. Weighting fidelity co-equal with recombination: Blue Prince rose
  83 → 87 (into the strong band, now second in Approach 1 behind Slay the Spire), Slay the Spire eased
  92 → 90 (its game-level followed its Approach 1 peak down, tying Into the Breach at the keystone
  floor and re-formatting its Approach 3 bullet to a single 90), Balatro eased 84 → 82, Against the
  Storm held at 86, and Monster Train fell to 79 out of Approach 1 while holding 80 in Approach 3, so
  it is not an Appendix cut. Added a **Forced draft, no skip** design principle and the transfer
