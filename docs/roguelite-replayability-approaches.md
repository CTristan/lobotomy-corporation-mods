...
**Ranking criterion:** games are judged by their relevance and applicability to building a LobCorp roguelite mod, which means how directly their lessons can be researched and lifted, not how deep the idea is in the abstract. Each game gets one calibration score, an anchored ordinal on a 0–100 scale, not a probability and not a per-lesson rating. The score is **game-level**, so a game that teaches several approaches keeps the same number across all of them (this is why dotAGE stays 86 whether you read it as Approach 2, 3, 4, or 6), and the approach tags show *where* it contributes, not *how much* per place. The bands are what carry meaning, and the number inside a band is a soft ordering, not a precise distance, so 88 over 86 means "same tier, roughly this order," not "two percent better."
...
- **Research:**
  ...
  - **dotAGE Elders (86).** It does asymmetric starts through Elders, leaders that each change a run's mechanics, which is a direct genre-proof of the patron-Sephirah idea inside a management roguelite. Study how an Elder reshapes the whole run's rules rather than just granting a bonus, because that's the bar a patron-Sephirah should clear. Little fails to transfer, because the genre and scope already match, so it's the load-bearing reference for this lever.
