...
namespace LobotomyCorporationMods.Common.Implementations.Facades
{
    ...
    {
        [ThreadStatic]
        private static UnityAdapterDictionary<string, IImageTestAdapter, Image> s_imagesDictionary;

        private static void CreateImageObjectIfNotExist(
        ...
        )
        {
            ...
            s_imagesDictionary = s_imagesDictionary.EnsureNotNullWithMethod(
            ...
            testAdapterParameters.Texture2DTestAdapter =
                testAdapterParameters.Texture2DTestAdapter.EnsureNotNullWithMethod(
                    () => new Texture2dTestAdapter()
                );
            ...
            if (string.IsNullOrEmpty(fileWithPath))
            {
                throw new InvalidOperationException(
                    "No image found with name " + imageParameters.ImageFilePath
                );
            }

            testAdapterParameters.Texture2DTestAdapter.LoadImage(
                fileManager.ReadAllBytes(fileWithPath)
            );

            var imageObject = managementSlot.CreateImageObjectTestAdapter(
                imageParameters,
        ...
        public static void HideImageObject(
            [NotNull] this ManagementSlot managementSlot,
            [NotNull] ImageParameters imageParameters,
            [NotNull] IFileManager fileManager,
            [CanBeNull] OptionalTestAdapterParameters testAdapterParameters = null
        ...
        public static bool IsAbnormalityWorkWindow(
        ...
        )
        {
            ...
            var isAbnormalityWorkWindow =
                commandWindow.CurrentSkill.IsNotNull()
                && commandWindow.CurrentWindowType == CommandType.Management;

            return isAbnormalityWorkWindow;
        }

        public static void UpdateAgentSlot(
        ...
        )
        {
            ...
            imageTestAdapter = imageTestAdapter.EnsureNotNullWithMethod(
                () => new ImageTestAdapter(agentSlot.WorkFilterFill)
            );
            textTestAdapter = textTestAdapter.EnsureNotNullWithMethod(
                () => new TextTestAdapter(agentSlot.WorkFilterText)
            );
