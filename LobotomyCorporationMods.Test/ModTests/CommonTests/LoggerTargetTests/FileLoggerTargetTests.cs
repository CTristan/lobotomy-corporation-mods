...
namespace LobotomyCorporationMods.Test.ModTests.CommonTests.LoggerTargetTests
{
    public sealed class FileLoggerTargetTests
    {
        ...
        public void Logging_message_creates_file_and_writes_to_file()
        {
            ...
            mockFileManager.Verify(manager => manager.GetFullPathForFile(ExpectedLogFilename), Times.Once);
            mockFileManager.Verify(manager => manager.AppendAllText(It.IsAny<string>(), ExpectedMessage), Times.Once);
        }
    }
}
