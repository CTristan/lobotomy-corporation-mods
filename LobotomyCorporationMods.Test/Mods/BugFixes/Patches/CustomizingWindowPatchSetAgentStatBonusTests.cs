...
namespace LobotomyCorporationMods.Test.Mods.BugFixes.Patches
{
    public sealed class CustomizingWindowPatchSetAgentStatBonusTests : BugFixesTests
    {
        ...
        public void When_upgrading_an_agent_we_should_always_use_the_original_stat_level_rather_than_the_stat_level_modified_by_bonuses()
        {
            ...
            var customizingWindow = TestUnityExtensions.CreateCustomizingWindow();
            ...
            var primaryStat = TestUnityExtensions.CreateWorkerPrimaryStat();
            ...
            var statBonus = TestUnityExtensions.CreateWorkerPrimaryStatBonus();
            statBonus.battle = BuffStatBonus;
            statBonus.hp = BuffStatBonus;
            statBonus.mental = BuffStatBonus;
            statBonus.work = BuffStatBonus;
            var statBuff = TestUnityExtensions.CreateUnitStatBuf(statBonus);
            var statBuffList = new List<UnitStatBuf> { statBuff };

            var agent = TestUnityExtensions.CreateAgentModel(primaryStat: primaryStat, statBufList: statBuffList);
            var data = TestUnityExtensions.CreateAgentData();
