...
namespace LobotomyCorporationMods.Test.Mods.NotifyWhenGiftReceived.Patches
{
    public sealed class UnitModelPatchAttachEgoGiftTests : NotifyWhenGiftReceivedTests
    {
        ...
        public void Attempting_to_replace_a_locked_gift_does_not_cause_a_notification(string agentName, string giftName, EGOgiftAttachRegion attachRegion)
        {
            // Arrange
            GetGift(giftName);
            ...
            // Assert
            NoticeAdapter.Verify(static adapter => adapter.Send(It.IsAny<string>(), It.IsAny<object[]>()), Times.Never);
        }
        ...
        [InlineData("Eke", "Our Galaxy", EGOgiftAttachRegion.RIBBORN)]
        public void Locked_gift_in_another_position_does_not_prevent_notification(string agentName, string giftName, EGOgiftAttachRegion attachRegion)
        {
            // Arrange
            GetGift(giftName);
            var unitModel = TestExtensions.CreateAgentModel(name: agentName);
            ...
            // Assert
            NoticeAdapter.Verify(static adapter => adapter.Send(It.IsAny<string>(), It.IsAny<object[]>()), Times.Once);
        }

        [Theory]
        [InlineData(DefaultAgentName, DefaultGiftName)]
        [InlineData("TestAgent", "TestGift")]
        public void Receiving_a_duplicate_gift_does_not_cause_a_notification(string agentName, string giftName)
        {
            // Arrange
        ...
        public void Receiving_a_gift_causes_a_notification(string agentName, string giftName, string expectedMessage)
        {
            // Arrange
            var gift = GetGift(giftName);
            var unitModel = TestExtensions.CreateAgentModel(name: agentName);
