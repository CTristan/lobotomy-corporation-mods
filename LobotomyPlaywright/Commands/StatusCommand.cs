...
public class StatusCommand
{
    ...
    public StatusCommand(IConfigManager configManager, ProcessManager processManager)
    {
    ...
    private static void PrintFormattedStatus(GameStatus status, int? pid, string host, int port, Dictionary<string, object>? gameState)
    {
        ...
        // TCP Server
        if (status == GameStatus.Starting || status == GameStatus.Ready || status == GameStatus.Unresponsive)
        {
