...
internal class CommandCommand
{
    ...
    public CommandCommand(IConfigManager configManager, Func<ITcpClient> tcpClientFactory)
    {
    ...
    private Dictionary<string, object>? BuildSetAgentStatsParams(string[] args)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(hp)) parameters["hp"] = float.Parse(hp);
            if (!string.IsNullOrEmpty(mental)) parameters["mental"] = float.Parse(mental);
            if (!string.IsNullOrEmpty(fortitude)) parameters["fortitude"] = int.Parse(fortitude);
            if (!string.IsNullOrEmpty(prudence)) parameters["prudence"] = int.Parse(prudence);
            if (!string.IsNullOrEmpty(temperance)) parameters["temperance"] = int.Parse(temperance);
            if (!string.IsNullOrEmpty(justice)) parameters["justice"] = int.Parse(justice);
        }
        catch (FormatException)
        {
    ...
    private void DisplayResponse(Dictionary<string, object> responseData)
    {
        ...
        {
            if (kvp.Key != "result" && kvp.Key != "error")
            {
