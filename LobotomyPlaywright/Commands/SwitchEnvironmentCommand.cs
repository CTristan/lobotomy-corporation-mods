...
namespace LobotomyPlaywright.Commands
{
    ...
    public class SwitchEnvironmentCommand(IConfigManager configManager, IFileSystem fileSystem, ProcessManager processManager)
    {
        ...
        public int Run(string[] args)
        {
            ...
            if (!isStatus)
            {
                ...
                if (target is not "debug" and not "release")
                {
            ...
            if (pids.Count > 0)
            {
                ...
                _ = _processManager.KillProcesses(pids, force: false);

                // Verify processes are stopped
                var remainingPids = _processManager.FindGameProcesses();
                if (remainingPids.Count > 0)
                {
                    _ = _processManager.KillProcesses(remainingPids, force: true);
                }
