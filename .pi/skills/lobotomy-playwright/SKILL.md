...
| `gameobjects` | `gameobject`, `go` | — | Discover/inspect/search any GameObject in the scene |
...
**GameObject-specific options:**

| Option | Default | Description |
|--------|---------|-------------|
| `--depth N` | 3 | Traverse depth for discovery (max: 10) |
| `--inspect PATH` | — | Inspect a specific GameObject by hierarchy path |
| `--detail full` | summary | Show reflected component fields (with `--inspect`) |
| `--search` | — | Switch to search mode |
| `--name TEXT` | — | Filter by name substring (with `--search`) |
| `--exact` | — | Match name exactly (with `--search`) |
| `--component TYPE` | — | Filter by component type name (with `--search`) |
| `--tag TAG` | — | Filter by tag (with `--search`) |
| `--active-only` | — | Only return active GameObjects (with `--search`) |
| `--dump` | — | Read local dump file instead of querying live |

**GameObject examples:**

```bash
# Discover scene hierarchy (default depth 3)
dotnet playwright query gameobjects

# Deeper traversal
dotnet playwright query gameobjects --depth 5

# Inspect a specific GameObject with component details
dotnet playwright query go --inspect "Canvas/Panel"

# Inspect with reflected field values
dotnet playwright query go --inspect "Canvas/Panel" --detail full

# Search for GameObjects by name
dotnet playwright query go --search --name "Button"

# Search for exact name match
dotnet playwright query go --search --name "Canvas" --exact

# Search by component type
dotnet playwright query go --search --component "Text"

# Search active objects with a specific tag
dotnet playwright query go --search --tag "MainCamera" --active-only

# Read offline dump (generated by plugin)
dotnet playwright query go --dump
```
